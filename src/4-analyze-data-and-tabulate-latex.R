...
tt(table1) |> 
  ...
  save_tt(glue("{data_dir}/output/freqtable-tiny-r.tex"),
          overwrite = TRUE)
...
datasummary( All(descripdata) ~ (N = NN) + 
             ...
             escape = F,
             output = glue("{data_dir}/output/descrip-r.tex"))
...
datasummary_correlation(descripdata, 
                        method = "pearspear",
                        output = glue("{data_dir}/output/corrtable-r.tex"),
                        escape = F)
...
modelsummary(models, 
                      ...
                      coef_map = cm,
                      gof_map = gm,
                      output = glue("{data_dir}/output/regression-r.tex"), 
                      escape = F
                      #booktabs = T
                      #add_rows = my_rows,
